...
Selain pertimbangan karakteristik dan dampak kesehatan, pemilihan PM\textsubscript{10} sebagai fokus prediksi juga didasarkan pada ketersediaan data. Seperti ditunjukkan pada Gambar \ref{gambar:missing-pm25}, kolom PM\textsubscript{2,5} memiliki persentase \textit{missing values} yang sangat tinggi mencapai 73\% (hanya 1.516 data valid dari total dataset), sedangkan PM\textsubscript{10} memiliki kelengkapan data yang jauh lebih baik (94\% valid). Keterbatasan data ini menjadikan pemodelan PM\textsubscript{2,5} tidak memungkinkan untuk dilakukan secara akurat menggunakan metode \textit{data-driven} pada rentang waktu 2010--2025, sehingga penelitian ini memfokuskan prediksi pada PM\textsubscript{10} yang memiliki kontinuitas data yang memadai untuk analisis deret waktu jangka panjang.

\begin{figure}[h]
  \centering
  \captionsetup{justification=centering}
      \includegraphics[width=0.9\textwidth]{image/gambar7.png}
  \caption{Statistik dataset ISPU Jakarta menunjukkan persentase \textit{missing values} untuk kolom pm25 mencapai 73\%}
  \label{gambar:missing-pm25}
\end{figure}
...
\textit{Random Forest} adalah algoritma \textit{ensemble learning} yang mengkombinasikan prediksi dari banyak pohon keputusan (\textit{decision trees}) untuk menghasilkan prediksi yang lebih akurat dan \textit{robust} \cite{alarsy2025pm25}. Setiap pohon dalam \textit{Random Forest} dilatih menggunakan subset acak dari data pelatihan (\textit{bootstrap sampling}) dan subset acak dari fitur pada setiap \textit{split node}.
...
Keunggulan \textit{Random Forest} untuk prediksi kualitas udara meliputi kemampuan menangani hubungan non-linier antar variabel, \textit{robust} terhadap \textit{outlier} dan \textit{noise}, dapat menangani data dengan banyak fitur tanpa memerlukan seleksi fitur eksplisit, serta memberikan estimasi \textit{feature importance} yang berguna untuk interpretasi \cite{abdallah2025predicting}.
...
\begin{figure}[h]
  \centering
  \captionsetup{justification=centering}
      \includegraphics[width=0.5\textwidth]{image/gambar2.png}
...
Beberapa penelitian terdahulu telah mengembangkan model prediksi kualitas udara menggunakan berbagai pendekatan \textit{machine learning}.
